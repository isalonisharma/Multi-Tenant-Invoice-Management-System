...
public class PdfGeneration {
	 ...
	 */
	public static ByteArrayInputStream generatePDF(Invoice invoice, List<InvoiceItem> invoiceItems,
			String organizationName, LocalDate dueDate, File organizationImage, String organizationDateFormat,
			String organizationCurrency) throws IOException, DocumentException {
		...
		Image image = null;
		try {
			image = Image.getInstance(organizationImage.getAbsolutePath());
		} catch (IOException e) {
		...
		Stream.of("Bill By", "Bill To", "Invoice Id : " + invoice.getId()).forEach(headerTitletop -> {
		...
		PdfPCell userNameCell = new PdfPCell(
				new Phrase(invoice.getUser().getFirstName() + " " + invoice.getUser().getLastName(),
		...
		PdfPCell clientNameCell = new PdfPCell(
				new Phrase(invoice.getClient().getFirstName() + " " + invoice.getClient().getLastName(),
						FontFactory.getFont(FontFactory.HELVETICA, 11)));
		...
		PdfPCell dateCell = new PdfPCell(new Phrase(
				"Date Placed : " + DateConversion.convertLocalDateTime(invoice.getDatePlaced(), organizationDateFormat),
				FontFactory.getFont(FontFactory.HELVETICA, 11)));
		...
		PdfPCell userEmailCell = new PdfPCell(
				new Phrase(invoice.getUser().getUsername(), FontFactory.getFont(FontFactory.HELVETICA, 11)));
		...
		PdfPCell clientEmailCell = new PdfPCell(
				new Phrase(invoice.getClient().getEmailId(), FontFactory.getFont(FontFactory.HELVETICA, 11)));
		...
		PdfPCell clientOrgCell = new PdfPCell(
				new Phrase(invoice.getClient().getOrganization(), FontFactory.getFont(FontFactory.HELVETICA, 11)));
		...
		for (InvoiceItem invoiceItem : invoiceItems) {
			...
			PdfPCell itemNameCell = new PdfPCell(new Phrase(invoiceItem.getItem().getName()));
			...
			PdfPCell unitPriceCell = new PdfPCell(new Phrase(Float.toString(invoiceItem.getItem().getRate())));
			...
			PdfPCell amountCell = new PdfPCell(
					new Phrase(Float.toString(invoiceItem.getItem().getRate() * invoiceItem.getQuantity())));
		...
		PdfPCell totalCell = new PdfPCell(new Phrase(organizationCurrency + " " + Double.toString(invoice.getAmount()),
